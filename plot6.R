...
# Subset to get just the entries for Los Angeles, California
NEI_LosAngeles <- NEI_subset[which(NEI_subset$fips == "06037"),]

# Create a table that summarizes the total emissions for each year.
annualEmissions <- ddply(NEI_Baltimore, "year", summarise, County = "Baltimore", Emissions = sum(Emissions))
annualEmissions <- rbind(annualEmissions, 
                         ddply(NEI_LosAngeles, "year", summarise, County = "Los Angeles", Emissions = sum(Emissions)))
...
g <- ggplot(annualEmissions, aes(year, Emissions, color=County, group=County))
...
g <- g + labs(title = "Baltimore City & Los Angeles Motor Vehicle Related PM2.5 Emissions, 1999-2008")
